...
pnpm dev                      # default world (static/games/terrain.jsonld)
...
## Plugins (ontology packs)

Installed plugin packs merge into the world graph **before** registration, so
plugin components, types, and entities behave exactly like world-authored
nodes (unknown fields warn-and-skip, type defaults merge, formulas derive).

- Packs live under `static/plugins/` as `<id>.jsonld` — a JSON-LD world fragment
  (`@context` + `@graph`).
- Discovery is via `static/plugins/manifest.json`:
  `{ "plugins": ["health", "enemy"] }`.
- The merge is additive and order-stable (world nodes first, then plugins in
  manifest order). A missing manifest or pack is a no-op, not an error.
- Seam: `mergePlugins()` in
  [ontology/source.ts](src/lib/engine/ontology/source.ts), called by
  `loadOntology()`.

```jsonc
// static/plugins/manifest.json
{ "plugins": ["health"] }
```

```jsonc
// static/plugins/health.jsonld
{ "@context": { "@vocab": "https://game.example/vocab/" },
  "@graph": [
...
Now any world in the app can use `Health` without declaring it. `playlab plugin
add` writes these files for you (see the plugin CLI); the seam works standalone.
