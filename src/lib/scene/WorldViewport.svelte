<script lang="ts">
  ...
  import AgentFocusLabelOverlay from '$lib/ui/AgentFocusLabelOverlay.svelte';
  import { agentFocus } from '$lib/engine/agent/agentFocus.svelte';
  ...
  <div class="viewport-canvas">
    <ContextMenu.Root>
      <ContextMenu.Trigger
        class={cn(
          'viewport-hit-target',
          ui.placementDraft !== null && 'placement-drop-target',
        )}
        role="region"
      ...
      {#if editMode}
        <ContextMenu.Content class="w-52">
          <ContextMenu.Group>
            ...
            >
              ...
              <ContextMenu.Shortcut
                >{shortcutLabels.duplicate}</ContextMenu.Shortcut
              >
            </ContextMenu.Item>
          </ContextMenu.Group>
          ...
          <ContextMenu.Group>
            <ContextMenu.Item
              disabled={!canFocus}
              onSelect={() => camera.focusSelection()}
            >
              Focus selection
...
<style>
  ...
  .viewport-view-controls {
    ...
    bottom: var(
      ...
      var(--chrome-bottom-outer, var(--float-inset))
    );
