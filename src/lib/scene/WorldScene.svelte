<script lang="ts">
  ...
  import AgentFocusLabelProjector from '$lib/scene/AgentFocusLabelProjector.svelte';
  ...
  import {
    ...
    createCamera2DState,
    resetCamera2DState,
  } from '$lib/engine/camera/camera2D';
  ...
  const childIds = $derived(
    new Set(world.entities.flatMap((e) => e.children ?? [])),
  );
  ...
  const showEditorGrid = $derived(
    ui.shellMode === 'edit' &&
      ui.chrome.grid &&
      (!worldProfile.is2d || playPlane === 'xz'),
  );
  const show2dPlaneGrid = $derived(
    worldProfile.is2d && ui.shellMode === 'edit' && ui.chrome.grid,
  );
  ...
  const editor2dSeedZoom = $derived.by(() => {
    const cam2d = world.getEntity(WORLD_PROFILE_ENTITY_ID)?.components
      .Camera2D as { zoom?: number } | undefined;
  ...
  function apply2dPlayCamera() {
    ...
    const pos = (
      entity?.components.Transform as { position?: [number, number, number] }
    )?.position;
  ...
  function resolveOrthoZoom(distance?: number): number {
    if (worldProfile.is2d)
      return normalizeOrthoZoom(editor2dSeedZoom, size.current.height);
    if (distance !== undefined && distance > 0.01) {
      return orthoZoomForPerspectiveFraming(
        distance,
        size.current.height,
        camera.fov,
      );
  ...
  function activateOrthographicViewport() {
    ...
    if (
      !Number.isFinite(
        cam.position.x + cam.position.y + cam.position.z + cam.zoom,
      )
    ) {
      return;
    }
  ...
  useTask(
    (delta) => {
      ...
      playCamera.applyLookDelta(
        look.x * speed * delta,
        -look.y * invertY * speed * delta,
      );
      invalidate();
    },
    {
  ...
  function captureOrbitSnapshot() {
    if (controls) {
      ...
      controls.getTarget(orbitTarget);
      const snap = {
        position: [orbitPos.x, orbitPos.y, orbitPos.z] as [
          number,
          number,
          number,
        ],
        target: [orbitTarget.x, orbitTarget.y, orbitTarget.z] as [
          number,
          number,
          number,
        ],
  ...
  {#if (useEditOrbitControls || usePlayCameraControls) && activeCam}
    <CameraControls
      ...
      minPolarAngle={useFollowCam
        ? followCfg.minPolarAngle
        : cfg.minPolarDeg * DEG2RAD}
      maxPolarAngle={useFollowCam
        ? followCfg.maxPolarAngle
        : cfg.maxPolarDeg * DEG2RAD}
      azimuthRotateSpeed={cfg.rotateSpeed}
      {polarRotateSpeed}
      dollySpeed={isOrtho
        ? cfg.dollySpeed * ORTHO_DOLLY_SPEED_MULT
        : cfg.dollySpeed}
...
>
  ...
  <T.Mesh
    ...
    onpointermove={(event: IntersectionEvent<PointerEvent>) => {
      if (ui.shellMode === 'edit' && ui.placementDraft)
  ...
  >
    ...
    {:else}
      <T.ShadowMaterial
        ...
        {...ui.shellMode === 'play' && ui.scene.shadows
          ? BACKDROP_POLYGON_OFFSET
          : {}}
      />
    {/if}
