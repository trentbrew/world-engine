<script lang="ts">
	...
	/** Nudge framing up when the bottom shelf is open so the mesh clears chrome. */
	const frameLift = $derived(
		ui.railRoute === 'objects' && ui.bottomChromeHeight > 0 ? 0.12 : 0
	);
	...
	$effect(() => {
		...
		if (!controls) return;
		const y = frameLift;
		void controls.setLookAt(4.2, 2.6 + y, 4.6, 0, 0.85 + y, 0, false);
