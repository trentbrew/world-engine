<script lang="ts">
	...
	const prompt = $derived(playerInteractPrompt.prompt);
	const show = $derived(
		ui.shellMode === 'play' &&
			!!prompt?.visible &&
			prompt.onScreen &&
...
{#if show && prompt}
	<div class="interact-prompt-layer" aria-live="polite">
		<div
			...
			role="status"
		>
			<kbd class="interact-prompt-key">{prompt.hint.replace(/^Press\s+/i, '')}</kbd>
			<span class="interact-prompt-label">{prompt.label}</span>
		</div>
	</div>
{/if}

<style>
	.interact-prompt-layer {
		position: absolute;
		inset: 0;
		z-index: 3;
		pointer-events: none;
