<script lang="ts">
  ...
  const bottomPaneHeight = $derived(
    showPanels && !!bottom ? ui.bottomPaneHeight : 0,
  );
  ...
  const showRightPanel = $derived(
    showPanels && rightPanelVisible && !!rightPanel,
  );
  const leftChromeWidth = $derived(
    railWidth + (showPanels ? ui.leftPanelWidth : 0),
  );
  const railBottomBand = $derived(
    railOnBottom ? RAIL_HEIGHT + VIEWPORT_FLOAT_INSET : 0,
  );
    ...
    >
      ...
      {#if showPanels && leftPanel}
        <aside class="app-left-panel">
          <div
            class="panel-shell chrome-float-card glass-panel-shell chrome-opacity-panel"
          >
            {@render leftPanel()}
          </div>
          <PanelResizeHandle
            edge="end"
            onResize={(delta) => ui.resizeLeftPanel(delta)}
          />
        </aside>
      {/if}
      {#if showRightPanel && rightPanel}
        <aside class="app-right-panel">
          <PanelResizeHandle
            edge="start"
            onResize={(delta) => ui.resizeRightPanel(delta)}
          />
          <div
            class="panel-shell chrome-float-card glass-panel-shell chrome-opacity-panel"
          >
...
<style>
  ...
  .app-left-panel {
    ...
    left: calc(var(--rail-width) + var(--chrome-edge));
    width: calc(
      var(--left-panel-width) + var(--chrome-rail-gap) + var(--chrome-panel-gap)
    );
    height: auto;
    min-height: var(--chrome-panel-min-height);
    max-height: calc(
      100% - var(--chrome-top-outer) - var(--bottom-chrome-height, 0px) -
        var(--chrome-bottom-outer) - var(--chrome-panel-bottom-slack)
    );
  ...
  .app-right-panel {
    ...
    right: 0;
    width: calc(
      var(--right-panel-width) + var(--chrome-edge) + var(--chrome-panel-gap)
    );
    height: auto;
    min-height: var(--chrome-panel-min-height);
    max-height: calc(
  ...
  .app-bottom-pane {
    position: absolute;
    left: calc(
      var(--rail-width) + var(--chrome-edge) + var(--chrome-rail-gap) +
        var(--left-panel-width) + var(--chrome-panel-gap)
    );
    right: calc(
      var(--right-panel-width) + var(--chrome-edge) + var(--chrome-panel-gap)
    );
  ...
  .app-shell.rail-position-bottom .app-left-panel {
    ...
    max-height: calc(
      100% - var(--chrome-top-outer) - var(--bottom-chrome-height, 0px) -
        var(--chrome-bottom-outer) - var(--rail-height) - var(--chrome-edge) -
  ...
  .app-shell.rail-position-bottom .app-bottom-pane {
    left: calc(
      var(--chrome-edge) + var(--chrome-rail-gap) + var(--left-panel-width) +
