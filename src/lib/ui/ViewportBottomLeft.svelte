<script lang="ts">
  ...
  const showCamera = $derived(viewportDebug.cameraHud);
  ...
  $effect(() => {
    ...
    if (!showCamera && playHudMenu.isOpen('camera')) playHudMenu.close();
...
>
  ...
  {/if}
  {#if showLiveControls && showCamera}
    <PlayCameraControls />
  {/if}
  {#if showStats}
...
<style>
  ...
  .viewport-play-hud--float {
    ...
    top: var(
      --chrome-top-outer,
      calc(var(--float-inset) + var(--doc-bar-chrome-height, 48px))
    );
