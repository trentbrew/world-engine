<script lang="ts" module>
	...
	export const buttonGroupVariants = tv({
		...
		variants: {
			...
			corners: {
				default: "",
				flat: "[&>[data-slot]]:rounded-none! [&>[data-slot]:not(:has(~[data-slot]))]:rounded-none! has-[select[aria-hidden=true]:last-child]:[&>[data-slot=select-trigger]:last-of-type]:rounded-none!",
			},
		},
		defaultVariants: {
	...
	export type ButtonGroupCorners = VariantProps<typeof buttonGroupVariants>["corners"];
</script>
...
<div
	...
	class={cn(buttonGroupVariants({ orientation, corners }), className)}
	{...restProps}
>
	{@render children?.()}
</div>
